...
struct mec_cct_config {
    uint16_t flags;
    uint8_t fr_clk_div;
    uint16_t cap_cfg[MEC_CCT_CAP_ID_MAX];
 };

inline uint32_t mec_hal_cct_fr_count_get(struct mec_cct_regs *regs)
{
    return regs->FR_COUNT;
}

inline void mec_hal_cct_fr_enable(struct mec_cct_regs *regs)
{
    regs->TCTRL |= MEC_BIT(MEC_CCT_TCTRL_FREN_Pos);
}

inline void mec_hal_cct_fr_disable(struct mec_cct_regs *regs)
{
    regs->TCTRL &= (uint32_t)~MEC_BIT(MEC_CCT_TCTRL_FREN_Pos);
}

inline void mec_hal_cct_fr_reset(struct mec_cct_regs *regs)
{
    regs->TCTRL |= MEC_BIT(MEC_CCT_TCTRL_FRRST_Pos);
}

inline uint32_t mec_hal_cct_cap_count(struct mec_cct_regs *regs, uint8_t capid)
{
    uintptr_t raddr = (uintptr_t)&regs->CAP0_CNT + (capid * 4u);

    return *(volatile uint32_t *)raddr;
}
...
void mec_hal_cct_clear_irq(struct mec_cct_regs *regs, uint32_t imask);
void mec_hal_cct_enable_irq(struct mec_cct_regs *regs, uint32_t ibm);
void mec_hal_cct_disable_irq(struct mec_cct_regs *regs, uint32_t ibm);
