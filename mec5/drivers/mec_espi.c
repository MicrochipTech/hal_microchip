...
static void set_supported_channels(uintptr_t ioc_base, uint32_t capabilities)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP0_OFS;
...
static void set_supported_max_freq(uintptr_t ioc_base, uint32_t caps)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP1_OFS;
 ...
 */
static uint32_t get_max_freq(uintptr_t ioc_base)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP1_OFS;
...
static void set_supported_io_modes(uintptr_t ioc_base, uint32_t caps)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP1_OFS;
 ...
 */
static uint32_t get_supported_io_modes(uintptr_t ioc_base)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP1_OFS;
 ...
 */
static void set_supported_alert_io_pin_mode(uintptr_t ioc_base, uint32_t caps)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP1_OFS;
...
static void set_pc_capabilities(uintptr_t ioc_base, uint32_t caps)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP_PC_OFS;
...
static uint32_t get_pc_max_pld_size(uintptr_t ioc_base)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP_PC_OFS;
...
static void set_vw_capabilities(uintptr_t ioc_base, uint32_t caps)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP_VW_OFS;
...
static uint32_t get_vw_groups_max_cnt(uintptr_t ioc_base)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP_VW_OFS;
    uint32_t v = mmcr8_rd(rb);
...
static void set_oob_capabilities(uintptr_t ioc_base, uint32_t caps)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP_OOB_OFS;
...
static uint32_t get_oob_pld_size(uintptr_t ioc_base)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP_OOB_OFS;
    uint32_t v = mmcr8_rd(rb);
...
static void set_fc_max_pld(uintptr_t ioc_base, uint32_t caps)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP_FC_OFS;
...
static uint32_t get_fc_pld_size(uintptr_t ioc_base)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP_FC_OFS;
    uint32_t v = mmcr8_rd(rb);
...
static void set_fc_shared_mode(uintptr_t ioc_base, uint32_t caps)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP_FC_OFS;
...
static uint32_t get_fc_shared_mode(uintptr_t ioc_base)
{
    uintptr_t rb = ioc_base + MEC_ESPI_CAP_FC_OFS;
    uint32_t v = mmcr8_rd(rb);
...
static void set_fc_capabilities(uintptr_t ioc_base, uint32_t caps)
{
    set_fc_max_pld(ioc_base, caps);
    set_fc_shared_mode(ioc_base, caps);
}
 ...
 */
static void set_pltrst_source(uintptr_t ioc_base, uint32_t caps)
{
    uintptr_t rb = ioc_base + MEC_ESPI_PLTRST_SRC_OFS;
...
/* ---- Public API ---- */
void mec_hal_espi_reset_change_clr(uintptr_t ioc_base)
{
    uintptr_t rb = ioc_base + MEC_ESPI_RESET_SR_OFS;
...
uint32_t mec_hal_espi_reset_state(uintptr_t ioc_base)
{
    uintptr_t rb = ioc_base + MEC_ESPI_RESET_SR_OFS;
    uint32_t v = mmcr8_rd(rb);
...
void mec_hal_espi_reset_change_intr_en(uintptr_t ioc_base, uint8_t enable)
{
    uintptr_t rb = ioc_base + MEC_ESPI_RESET_IER_OFS;
...
int mec_hal_espi_init(struct mec_espi_config *cfg)
{
    ...
    uintptr_t ioc_base = cfg->iobase;
    uint32_t girq_en = 0u;

    set_supported_channels(ioc_base, cfg->capabilities);
    set_supported_max_freq(ioc_base, cfg->capabilities);
    set_supported_io_modes(ioc_base, cfg->capabilities);
    set_supported_alert_io_pin_mode(ioc_base, cfg->capabilities);
    set_pc_capabilities(ioc_base, cfg->capabilities);
    set_vw_capabilities(ioc_base, cfg->capabilities);
    set_oob_capabilities(ioc_base, cfg->capabilities);
    set_fc_capabilities(ioc_base, cfg->capabilities);
    set_pltrst_source(ioc_base, cfg->capabilities);

    mec_hal_espi_reset_change_intr_en(ioc_base, 0);
    mec_hal_espi_reset_change_clr(ioc_base);
    ...
    mec_hal_girq_bm_en(MEC_ESPI_GIRQ, girq_en, 1u);
 ...
 */
void mec_hal_espi_activate(uintptr_t ioc_base, uint8_t enable)
{
    uintptr_t rb = ioc_base + MEC_ESPI_ACTV_OFS;
...
int mec_hal_espi_capability_set(uintptr_t ioc_base, enum mec_espi_global_cap cap, uint32_t cfg)
{
    if (ioc_base == 0) {
        return MEC_RET_ERR_INVAL;
    }
    ...
    case MEC_ESPI_CAP_MAX_FREQ:
        set_supported_max_freq(ioc_base, cfg);
        break;
    case MEC_ESPI_CAP_IO_MODE:
        set_supported_io_modes(ioc_base, cfg);
        break;
    case MEC_ESPI_CAP_ALERT_OD:
        set_supported_alert_io_pin_mode(ioc_base, cfg);
        break;
    case MEC_ESPI_CAP_PERIPH_CHAN:
        if (cfg & MEC_BIT(MEC_ESPI_CFG_PERIPH_CHAN_SUP_POS)) {
            mmcr8_set_bit(ioc_base + MEC_ESPI_CAP0_OFS, MEC_ESPI_CAP0_PC_SUPP_POS);
        } else {
            mmcr8_clr_bit(ioc_base + MEC_ESPI_CAP0_OFS, MEC_ESPI_CAP0_PC_SUPP_POS);
        }
        break;
    case MEC_ESPI_CAP_PC_MAX_PLD_SIZE:
        set_pc_capabilities(ioc_base, cfg);
        break;
    case  MEC_ESPI_CAP_VWIRE_CHAN:
        if (cfg & MEC_BIT(MEC_ESPI_CFG_VW_CHAN_SUP_POS)) {
            mmcr8_set_bit(ioc_base + MEC_ESPI_CAP0_OFS, MEC_ESPI_CAP0_VW_SUPP_POS);
        } else {
            mmcr8_clr_bit(ioc_base + MEC_ESPI_CAP0_OFS, MEC_ESPI_CAP0_VW_SUPP_POS);
        }
        break;
    case MEC_ESPI_CAP_MAX_VW_COUNT:
        set_vw_capabilities(ioc_base, cfg);
        break;
    case MEC_ESPI_CAP_OOB_CHAN:
        if (cfg & MEC_BIT(MEC_ESPI_CFG_OOB_CHAN_SUP_POS)) {
            mmcr8_set_bit(ioc_base + MEC_ESPI_CAP0_OFS, MEC_ESPI_CAP0_OOB_SUPP_POS);
        } else {
            mmcr8_clr_bit(ioc_base + MEC_ESPI_CAP0_OFS, MEC_ESPI_CAP0_OOB_SUPP_POS);
        }
        break;
    case MEC_ESPI_CAP_OOB_MAX_PLD_SIZE:
        set_oob_capabilities(ioc_base, cfg);
        break;
    case MEC_ESPI_CAP_FLASH_CHAN:
        if (cfg & MEC_BIT(MEC_ESPI_CFG_FLASH_CHAN_SUP_POS)) {
            mmcr8_set_bit(ioc_base + MEC_ESPI_CAP0_OFS, MEC_ESPI_CAP0_FC_SUPP_POS);
        } else {
            mmcr8_set_bit(ioc_base + MEC_ESPI_CAP0_OFS, MEC_ESPI_CAP0_FC_SUPP_POS);
        }
        break;
    case MEC_ESPI_CAP_FC_MAX_PLD_SIZE:
        set_fc_max_pld(ioc_base, cfg);
        break;
    case MEC_ESPI_CAP_FC_SHARING:
        set_fc_shared_mode(ioc_base, cfg);
        break;
    default:
        return MEC_RET_ERR_INVAL;
    }
