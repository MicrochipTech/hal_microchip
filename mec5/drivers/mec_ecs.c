...
{
    if (ahb_err_enable) {
        ECS->AERRC &= (uint32_t)~BIT(ECS_AERRC_CAP_Pos);
    } else {
        ECS->AERRC |= BIT(ECS_AERRC_CAP_Pos);
    }
}
...
{
    if (feature < 32) {
        if (ECS->FEAT_LOCK & BIT(feature)) {
            return 1;
        }
    } else if (feature < 64) {
        feature -= 32;
        if (ECS->MISC_LOCK & BIT(feature)) {
            return 1;
        }
    }
...
{
    if (enable) {
        ECS->ETM_CTRL |= BIT(ECS_ETM_CTRL_TRACE_EN_Pos);
    } else {
        ECS->ETM_CTRL &= ~BIT(ECS_ETM_CTRL_TRACE_EN_Pos);
    }
}
...
void mec_ecs_debug_port(enum mec_debug_mode mode)
{
    ...
    case MEC_DEBUG_MODE_DISABLE:
        msk = ~BIT(ECS_DBG_CTRL_EN_Pos);
        val = 0u;
        break;
    case MEC_DEBUG_MODE_JTAG:
        msk = ECS_DBG_CTRL_CFG_Msk;
        val = BIT(ECS_DBG_CTRL_EN_Pos) | (uint32_t)ECS_DBG_CTRL_CFG_JTAG;
        break;
    case MEC_DEBUG_MODE_SWD:
        msk = ECS_DBG_CTRL_CFG_Msk;
        val = BIT(ECS_DBG_CTRL_EN_Pos) | (uint32_t)ECS_DBG_CTRL_CFG_SWD_ONLY;
        break;
    case MEC_DEBUG_MODE_SWD_SWV:
        msk = ECS_DBG_CTRL_CFG_Msk;
        val = BIT(ECS_DBG_CTRL_EN_Pos) | (uint32_t)ECS_DBG_CTRL_CFG_SWD_SWV;
        break;
    default:
        return;
    }
