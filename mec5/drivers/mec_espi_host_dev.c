...
int mec_hal_espi_iobar_cfg(uintptr_t ioc_base, uint8_t ldn, uint16_t io_base, uint8_t enable)
{
    uintptr_t rb = ioc_base;
    ...
    if ((ioc_base == 0u) || (ldn >= MEC_ESPI_LDN_MAX)) {
...
{
    return mec_hal_espi_iobar_is_enabled(MEC_ESPI_IOC_BASE, ldn);
}
...
{
    return mec_hal_espi_mbar_is_enabled(MEC_ESPI_MC_BASE, ldn);
}
