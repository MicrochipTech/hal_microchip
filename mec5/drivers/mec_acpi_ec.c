...
{
    const struct mec_acpi_ec_info *info = find_acpi_ec_info(regbase);
    ...
    mec_hal_acpi_ec_girq_dis(regbase, MEC_ACPI_EC_IBF_IRQ | MEC_ACPI_EC_OBE_IRQ);
    mec_hal_pcr_clr_blk_slp_en(info->pcr_id);

    if ((flags & MEC_ACPI_EC_RESET) != 0) {
        mec_hal_pcr_blk_reset(info->pcr_id);
    } else {
        mmcr32_rd(regbase + MEC_AEC_OS_DATA0_OFS); /* clear OBF, set OBE */
        mmcr32_rd(regbase + MEC_AEC_H2E_DATA0_OFS); /* clear IBF */
    }

    mec_hal_acpi_ec_girq_clr(regbase, MEC_ACPI_EC_IBF_IRQ | MEC_ACPI_EC_OBE_IRQ);

    if ((flags & MEC_ACPI_EC_4BYTE_MODE) != 0) {
        mmcr8_set_bit(regbase + MEC_AEC_BC_OFS, MEC_AEC_BC_4BEN_POS);
    } else {
        mmcr8_clr_bit(regbase + MEC_AEC_BC_OFS, MEC_AEC_BC_4BEN_POS);
    }

    if ((flags & MEC_ACPI_EC_UD0A_SET) != 0) {
        if ((flags & MEC_ACPI_EC_UD0A_ONE) != 0) {
            mmcr8_set_bit(regbase + MEC_AEC_SR_OFS, MEC_AEC_SR_UD0A_POS);
        } else {
            mmcr8_clr_bit(regbase + MEC_AEC_SR_OFS, MEC_AEC_SR_UD0A_POS);
        }
    }

    if ((flags & MEC_ACPI_EC_UD1A_SET) != 0) {
        if ((flags & MEC_ACPI_EC_UD1A_ONE) != 0) {
            mmcr8_set_bit(regbase + MEC_AEC_SR_OFS, MEC_AEC_SR_UD1A_POS);
        } else {
            mmcr8_clr_bit(regbase + MEC_AEC_SR_OFS, MEC_AEC_SR_UD1A_POS);
        }
    }

    mec_hal_acpi_ec_girq_dis(regbase, flags);
...
uint32_t mec_hal_acpi_ec_girq_result(uintptr_t regbase)
{
    ...
    if ((temp & MEC_BIT(ibf_bit)) != 0) {
        result |= MEC_ACPI_EC_IBF_IRQ;
    }

    if ((temp & MEC_BIT(obe_bit)) != 0) {
...
{
    uintptr_t espi_io_base = (uintptr_t)(MEC_ESPI_IO_BASE);
    uintptr_t espi_mem_base = (uintptr_t)(MEC_ESPI_MEM_BASE);
    const struct mec_acpi_ec_info *info = find_acpi_ec_info(regbase);
...
int mec_hal_acpi_ec_is_4byte_mode(uintptr_t regbase)
{
    ...
    if (mmcr8_test_bit(regbase + MEC_AEC_BC_OFS, MEC_AEC_BC_4BEN_POS) != 0) {
        return 1;
    }
...
#endif
    return mmcr8_rd(regbase + MEC_AEC_SR_OFS);
}
...
#endif
    mmcr8_wr(val, regbase + MEC_AEC_SR_OFS);
}
...
#endif
    mmcr8_set_bits(regbase + MEC_AEC_SR_OFS, val);
}
...
#endif
    mmcr8_update_field(regbase + MEC_AEC_SR_OFS, val, msk);
}
...
#endif
    return mmcr8_test_bit(regbase + MEC_AEC_SR_OFS, MEC_AEC_SR_OBF_POS);
}
...
#endif
    return mmcr8_test_bit(regbase + MEC_AEC_SR_OFS, MEC_AEC_SR_IBF_POS);
}
...
#endif
    return mmcr32_rd(regbase + MEC_AEC_H2E_DATA0_OFS);
}
...
#endif
    mmcr32_wr(data, regbase + MEC_AEC_H2E_DATA0_OFS);
}
...
#endif
    uint32_t raddr = regbase + MEC_AEC_H2E_DATA0_OFS + (offset & 0x3u);
    
    return mmcr8_rd(raddr);
}
...
#endif
    uint32_t addr = regbase + MEC_AEC_H2E_DATA0_OFS + (offset & 0x3u);
...
#endif
    return mmcr32_rd(regbase + MEC_AEC_E2H_DATA0_OFS);
}
...
#endif
    mmcr32_wr(data, regbase + MEC_AEC_E2H_DATA0_OFS);
}
...
#endif
    uint32_t addr = regbase + MEC_AEC_E2H_DATA0_OFS + (uint32_t)(offset & 0x3u);

    return mmcr8_rd(addr);
}
