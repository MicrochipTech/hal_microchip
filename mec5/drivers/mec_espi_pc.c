...
static uint32_t xlat_intr_to_hw(uint32_t bitmap)
{
    ...
    if (bitmap & MEC_BIT(MEC_ESPI_PC_INTR_CHEN_CHG_POS)) {
        hwbm |= MEC_BIT(MEC_ESPI_PC_IER_CHEN_CHG_POS);
    }

    if (bitmap & MEC_BIT(MEC_ESPI_PC_INTR_BMEN_CHG_POS)) {
        hwbm |= MEC_BIT(MEC_ESPI_PC_IER_BMEN_CHG_POS);
    }

    if (bitmap & MEC_BIT(MEC_ESPI_PC_INTR_BERR_POS)) {
        hwbm |= MEC_BIT(MEC_ESPI_PC_IER_ABERR_POS);
    }
...
uint32_t mec_hal_espi_pc_en_status(uintptr_t iobase)
{
    ...
    v &= (MEC_BIT(MEC_ESPI_PC_SR_CHEN_STATE_POS) | MEC_BIT(MEC_ESPI_PC_SR_CHEN_CHG_POS));
    v >>= MEC_ESPI_PC_SR_CHEN_STATE_POS;
...
uint32_t mec_hal_espi_pc_bm_status(uintptr_t iobase)
{
    ...
    v &= (MEC_BIT(MEC_ESPI_PC_SR_BMEN_STATE_POS) | MEC_BIT(MEC_ESPI_PC_SR_BMEN_CHG_POS));
    v >>= MEC_ESPI_PC_SR_BMEN_STATE_POS;
...
uint32_t mec_hal_espi_pc_status(uintptr_t iobase)
{
    ...
    if ((v & MEC_BIT(MEC_ESPI_PC_SR_ABERR_POS)) != 0) {
        sts |= MEC_BIT(MEC_ESPI_PC_ISTS_BERR_POS);
    }
    if ((v & MEC_BIT(MEC_ESPI_PC_SR_CHEN_CHG_POS)) != 0) {
        sts |= MEC_BIT(MEC_ESPI_PC_ISTS_CHEN_CHG_POS);
    }
    if ((v & MEC_BIT(MEC_ESPI_PC_SR_CHEN_STATE_POS)) != 0) {
        sts |= MEC_BIT(MEC_ESPI_PC_ISTS_CHEN_STATE_POS);
    }
    if ((v & MEC_BIT(MEC_ESPI_PC_SR_BMEN_STATE_POS)) != 0) {
        sts |= MEC_BIT(MEC_ESPI_PC_ISTS_BMEN_STATE_POS);
    }
    if ((v & MEC_BIT(MEC_ESPI_PC_SR_BMEN_CHG_POS)) != 0) {
...
{
    mmcr32_wr(MEC_ESPI_PC_SR_ALL_MSK, iobase + MEC_ESPI_PC_SR_OFS);
}
...
void mec_hal_espi_pc_last_cycle(uintptr_t iobase, struct mec_espi_pc_last_cycle *lc)
{
    ...
    lc->len = (uint16_t)MEC_ESPI_PC_LC_LTT_LEN_GET(v);
    lc->cycle_type = (uint8_t)MEC_ESPI_PC_LC_LTT_TYPE_GET(v);
    lc->tag = (uint8_t)MEC_ESPI_PC_LC_LTT_TAG_GET(v);
}
