...
#define MEC_FIELD_GET(mask, value)  \
        (((uint32_t)(value) & (uint32_t)(mask)) / MEC_LSB_GET((uint32_t)mask))
#endif

#ifndef MEC_FIELD_PREP
#define MEC_FIELD_PREP(mask, value) \
        ((((uint32_t)value) * MEC_LSB_GET((uint32_t)(mask))) & (uint32_t)(mask))
#endif
